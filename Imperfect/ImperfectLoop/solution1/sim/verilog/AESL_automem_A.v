...
//------------------------Parameter----------------------
localparam
    TV_IN  = "../tv/cdatafile/c.loop_imperfect.autotvin_A.dat", 
    TV_OUT = "../tv/rtldatafile/rtl.loop_imperfect.autotvout_A.dat"; 
//------------------------Local signal-------------------
parameter DATA_WIDTH = 32'd 5;
...
always @ (posedge clk or rst) begin
    if(rst === 1) begin
	  ...
	  if((we0[0] == 0) && (ce0 == 1) && (ce1 == 1) && (we1[0] == 1) && (address0 == address1))
	      dout0[0*8+4:0*8] <= #DLY din1[0*8+4:0*8];
	  else if (ce0 == 1)
	      dout0[0*8+4:0*8] <= #DLY mem[address0][0*8+4:0*8];
        else ;
    end
end
...
always @ (posedge clk or rst) begin
    if(rst === 1) begin
	  ...
	  if((we0[0] == 1) && (ce0 == 1) && (ce1 == 1) && (we1[0] == 0) && (address0 == address1))
            dout1[0*8+4:0*8] <= #DLY din0[0*8+4:0*8];
	  else if (ce1 == 1)
            dout1[0*8+4:0*8] <= #DLY mem[address1][0*8+4:0*8];
        else ;
    end
end
...
always @ (posedge clk) begin
    if((we0[0] == 1) && (ce0 == 1) && (ce1 == 1) && (we1[0] == 1) && (address0 == address1))
        mem[address0][0*8+4:0*8] <= #DLY din1[0*8+4:0*8];
    else if ((we0[0] == 1) && (ce0 == 1))
        mem[address0][0*8+4:0*8] <= #DLY din0[0*8+4:0*8];
end

always @ (posedge clk) begin
    if ((ce1 == 1) && (we1[0] == 1))
        mem[address1][0*8+4:0*8] <= #DLY din1[0*8+4:0*8];
end
